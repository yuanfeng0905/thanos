...
func (i Stats) CriticalErr() error {
	...
	if i.CompleteOutsideChunks > 0 {
		errMsg += fmt.Sprintf("found %d chunks completely outside the block time range.", i.CompleteOutsideChunks)
	}
...
func Repair(dir string, id ulid.ULID, ignoreChkFns ...ignoreFnType) (resid ulid.ULID, err error) {
...
func IgnoreCompleteOutsideChunk(mint, maxt int64, _ *chunks.Meta, curr *chunks.Meta) (bool, error) {
...
func IgnoreIssue347OutsideChunk(mint, maxt int64, _ *chunks.Meta, curr *chunks.Meta) (bool, error) {
	if curr.MinTime == maxt && curr.MaxTime > maxt {
...
func IgnoreDuplicateOutsideChunk(mint, maxt int64, prev *chunks.Meta, curr *chunks.Meta) (bool, error) {
	if prev == nil {
		return false, nil
	}
	...
	// the current one.
	if curr.MinTime != prev.MinTime || curr.MaxTime != prev.MaxTime {
		return false, errors.Errorf("non-sequential chunks not equal: [%d, %d] and [%d, %d]",
			prev.MaxTime, prev.MaxTime, curr.MinTime, curr.MaxTime)
	}
	ca := crc32.Checksum(prev.Chunk.Bytes(), castagnoli)
